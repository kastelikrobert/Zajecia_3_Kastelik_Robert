...
#pragma region Venus
	...
	//
	load_targa("ven0aaa2.tga", width, height,format ,type,pixels);
...
#pragma region Ziemia
	...
	load_targa("ear0xuu2.tga", width, height, format, type, pixels);
...
#pragma region Mars
	...
	// plik: http://maps.jpl.nasa.gov/pix/mar0kuu2.tif
	 load_targa("mar0kuu2.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku mar0kuu2.tif");
		exit(0);
	}
...
#pragma region Merkury

	//MERKURY
	load_targa("mer0muu2.tga", width, height, format, type, pixels);
